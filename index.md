...
<span class="proof-title">*注记*. </span>若您使用 uv 管理 Python，请在渲染前确保命令行正确激活 uv 环境，或使用 `uv run quarto render`．
...
<span class="proof-title">*注记*. </span>SageMath 在 Windows 原生环境下安装极为困难，我们建议通过直接编写 `.ipynb` 在 VSCode 中通过插件远程连接 WSL 中的 Jupyter 服务器使用 SageMath 内核．
